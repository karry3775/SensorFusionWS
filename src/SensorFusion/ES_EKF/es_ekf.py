...
gnss_i = 0
lidar_i = 0
print('PPPPPPPPPPPP===',q_est[0])
##### Sensor data Availability check:
def is_avl(s_t,t):
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    dp  = dx[0:3] #error state of position
    dv  = dx[3:6] #error state of velocity
    dth = dx[6:9] #error state of theta
    
    p_check = p_check + dp
    v_check = v_check + dv
    q_check = Quaternion(axis_angle=dth).quat_mult(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    #print(GNSSavailable )
    #print(Lidaravailable)
    sensorVar_gnss= var_gnss*np.eye(3)
    sensorVar_lidar= var_lidar*np.eye(3)
    
    #if GNSSavailable != -1: #valid gnss data
    #    p_check, v_check, q_check, p_cov_check=measurement_update(sensorVar_gnss, p_cov_check, gnss.data[GNSSavailable], p_check, v_check, q_check)
    
    if Lidaravailable != -1: #valid LIDAR data
        p_check, v_check, q_check, p_cov_check=measurement_update(sensorVar_lidar, p_cov_check, lidar.data[Lidaravailable], p_check, v_check, q_check)
